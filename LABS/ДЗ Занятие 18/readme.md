...
#### [Для S1](ShowSTPS12)
#### [Для S2](ShowSTPS22)
##### Почему протокол spanning-tree заменяет ранее заблокированный порт на назначенный порт и блокирует порт, который был назначенным портом на другом коммутаторе? Потому что стоимость порта (18) теперь меньше на стороне S1. Первоначальное сравнение идет по стоимости порта.
### Шаг 4:	Удалите изменения стоимости порта.
##### S1(config)# interface f0/4
##### S1(config-if)# no spanning-tree vlan 1 cost 18
##### Через 30 сек состояние портов на S1 и S2 вернулось в первоначальное состояние.
## Часть 4:	Наблюдение за процессом выбора протоколом STP порта, исходя из приоритета портов
#### Включение интерфейсов F0/1 и F0/3 на всех коммутаторах, выполнение комманд:
##### int range f0/1, f0/3
##### no sh
#### Результаты выполнения команды sh spanning-tree для каждого из коммутаторов:
#### [Для S1](ShowSTPS13)
#### [Для S2](ShowSTPS23)
##### На коммутаторе S1 из портов F0/3 и F0/4 в сторону root коммутатора S3 заблокирован порт F0/4, т.к. у него выше приоритет, (128.4) а остальные параметры совпадают:BID и cost. На коммутаторе S2 из портов F0/3 и F0/4 в сторону root коммутатора S3 заблокирован порт F0/4, т.к. у него выше приоритет (128.4), а остальные параметры совпадают:BID и cost.
