...
###### R2(config-if)#int loopback1
###### R2(config-if)#ip ospf 56 area 0
##### f.	Убедитесь, что OSPFv2 работает между маршрутизаторами. Выполните команду, чтобы убедиться, что R1 и R2 сформировали смежность: R2(config-if)#do sh ip ospf ne
