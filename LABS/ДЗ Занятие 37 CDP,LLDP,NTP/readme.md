...
##### Проверка результатов настройки выполнением команды sh ntp status на R1:
![](shntpstatR1.png)
##### Настроенный stratum- 4, источник -внутренняя настройка (127.127.1.1)
##### Проверка результатов настройки выполнением команды sh ntp associations на R1:
![](shntpassR1.png)
...
##### Проверка результатов настройки на примере коммутатора S1. Результат вывода команд sh clock, sh ntp as, sh ntp stat:
![](shntpS1.png)
##### Источник времени 10.22.0.1 - IP-адрес интерфейса маршрутизатора R1. Stratum источника -4. Часы синхронизированы, текущий Stratum самого коммутатора S1- в CPT указан как 16, в действительности должен быть 5.
#### b.	Выполните соответствующую команду на S1 и S2, чтобы просмотреть настроенное время и сравнить ранее записанное время.
##### Проверка настроек с помощью команды sh clock detail, на примере коммутатора S1:
###### S1#sh clock det
###### 17:7:59.532 UTC Sun Dec 14 2025
###### Time source is NTP
##### Время и дата получено от источника по NTP.
## Вопрос для повторения
### Для каких интерфейсов в пределах сети не следует использовать протоколы обнаружения сетевых ресурсов? Поясните ответ.
#### Для интерфейсов оборудования на периметре сети, которые подключены к внешним сетям (например, к интернету от провайдера). Чтобы не передавать во внешние сети информацию об конфигурации устройств внутренней сети.
